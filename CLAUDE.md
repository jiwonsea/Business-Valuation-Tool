...
- Monte Carlo negative equity is preserved in full-distribution statistics (mean, percentiles). Histogram display filters negatives out. `pct_negative` counts true negatives before any filtering. Do NOT clamp `ps = max(ps, 0)` — this upward-biases all statistics.
