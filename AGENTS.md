...
- Ignore generated outputs by default: root `*.xlsx`, `logs/`, `valuation-results/`, `test_output/`, `output/calibration/`, `.cache/`, `.Codex-octopus/`.
...
- `ai/` — LLM-based segment classification, peer recommendation, scenario design (Codex Sonnet 4).
...
- **Cowork/Linux-VM bridge Git reads**: Use `git --no-optional-locks ...` for read-only bridge commands to prevent a stale `index.lock`; local Git writes are unaffected.
...
- Forecast tests run from the repository root: `pytest forecast/tests/ -q`. `*_FROZEN.md` files are immutable; the integrity gate must satisfy `checked == passed == len(CONVENTION_PROFILES)`, `supported_skipped == 0`, and `failures == []` (mapping entries are report filenames, including each A/B report).
