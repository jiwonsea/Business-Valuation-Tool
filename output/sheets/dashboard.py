...
def sheet_dashboard(ctx: Ctx):
    ...
    write_cell(
        ...
        1,
        1,
        f"{ctx.vi.company.name} 기업가치평가 Dashboard",
    ...
    write_cell(
        ...
        2,
        1,
        f"분석일: {ctx.vi.company.analysis_date}  |  {method_desc}",
        font=NOTE_FONT,
    )
    ...
    write_cell(ws, r, 1, primary_label, font=Font(bold=True, size=14, color=NAVY))
    write_cell(
        ws,
        r,
        2,
        f"{primary_value:,}{ctx.currency_sym}",
        font=Font(bold=True, size=18, color="27AE60"),
        fill=GREEN_FILL,
    )
    ...
    if ctx.sc_codes and ctx.result.scenarios:
        r += 2
        write_cell(ws, r, 1, "시나리오별 주당 가치", font=SECTION_FONT)
        r += 1
        ...
        write_cell(
            ws, r, 2, ctx.result.weighted_value, fmt=NUM_FMT, bold=True, fill=GREEN_FILL
        )
    ...
    if ctx.method == "ddm" and ctx.result.ddm:
        ddm = ctx.result.ddm
        write_cell(ws, r, 1, "DDM 밸류에이션 요약", font=SECTION_FONT)
        r += 1
        ...
        rim = ctx.result.rim
        write_cell(ws, r, 1, "RIM 밸류에이션 요약", font=SECTION_FONT)
        r += 1
        items = [
        ...
        nav = ctx.result.nav
        write_cell(ws, r, 1, "NAV 밸류에이션 요약", font=SECTION_FONT)
        r += 1
        items = [
        ...
        write_cell(ws, r, 1, "Multiples 밸류에이션 요약", font=SECTION_FONT)
        r += 1
        items = [
        ...
        for label, val in items:
            ...
            is_ev = label == "DCF EV"
            write_cell(
                ws, r, 2, val, fill=GREEN_FILL if is_ev else BLUE_FILL, bold=is_ev
            )
            r += 1
        ...
        section_title = (
            "SOTP 밸류에이션 구성 (Mixed)"
            if has_mixed_dash
            else f"Enterprise Value 구성 ({ctx.unit})"
        )
        write_cell(ws, r, 1, section_title, font=SECTION_FONT)
        r += 1
        ev_data_start = r
        active_segs = []
        if ctx.result.sotp:
            active_segs = [
                ...
                if ctx.result.sotp.get(c) and ctx.result.sotp[c].ev > 0
            ]
        ...
        write_cell(
            ws, r, 2, ctx.result.total_ev, fmt=NUM_FMT, bold=True, fill=GREEN_FILL
        )
    ...
    write_cell(ws, r, 1, f"핵심 재무지표 ({ctx.by})", font=SECTION_FONT)
    r += 1
    ...
    if ctx.result.cross_validations:
        r += 1
        write_cell(ws, r, 1, "멀티플 교차검증 (Cross-Validation)", font=SECTION_FONT)
        r += 1
        cv_headers = [
            "방법론",
            "지표값",
    ...
    if ctx.result.cross_validations:
        ...
        dcf_val = next(
            (cv.per_share for cv in ctx.result.cross_validations if cv.method == "DCF"),
            None,
        )
    ...
    if ctx.result.market_comparison and ctx.result.market_comparison.market_price > 0:
        ...
        r += 2
        write_cell(ws, r, 1, "시장가격 비교", font=SECTION_FONT)
        r += 1
        write_cell(ws, r, 1, "내재가치 (주당)")
        write_cell(ws, r, 2, mc.intrinsic_value, fmt=NUM_FMT)
        r += 1
        write_cell(ws, r, 1, "현재 시장가")
        write_cell(ws, r, 2, mc.market_price, fmt=NUM_FMT)
        r += 1
    ...
    if ctx.result.gap_diagnostic:
        ...
        r += 2
        write_cell(ws, r, 1, "역방향 DCF 진단", font=SECTION_FONT)
        r += 1
        ...
        write_cell(
            ...
            r,
            2,
            _LABELS.get(gd.category, gd.category),
            font=Font(bold=True, color=cat_color),
        )
        r += 1
        write_cell(ws, r, 1, "괴리율")
        write_cell(ws, r, 2, f"{abs(gd.gap_pct):.1f}% ({gd.direction})")
        r += 1
        if gd.implied_wacc is not None:
            write_cell(ws, r, 1, "시장 내재 WACC")
            write_cell(ws, r, 2, f"{gd.implied_wacc:.2f}%")
            r += 1
        if gd.implied_tgr is not None:
            write_cell(ws, r, 1, "시장 내재 TGR")
            write_cell(ws, r, 2, f"{gd.implied_tgr:.2f}%")
            r += 1
        if gd.implied_growth_mult is not None:
            write_cell(ws, r, 1, "시장 내재 성장배수")
            write_cell(ws, r, 2, f"{gd.implied_growth_mult:.2f}x")
            r += 1
        if not gd.reconcilable:
            write_cell(
                ...
                1,
                "⚠ EBITDA DCF로 설명 불가",
                font=Font(bold=True, color="E74C3C"),
            )
            r += 1
        if gd.suggestions:
            for s in gd.suggestions[:3]:
                write_cell(ws, r, 1, s, font=Font(italic=True, size=9))
                r += 1
        if gd.explanation:
            write_cell(ws, r, 1, "상세 분석", font=Font(bold=True))
            write_cell(ws, r, 2, gd.explanation)
            r += 1
    ...
    if ctx.result.monte_carlo:
        ...
        r += 2
        write_cell(
            ws, r, 1, f"Monte Carlo 시뮬레이션 ({mc.n_sims:,}회)", font=SECTION_FONT
        )
        r += 1

        # MC Input Assumptions
        write_cell(
            ws, r, 1, "MC 입력 분포 가정 (Input Assumptions)", font=Font(bold=True)
        )
        r += 1
        ...
        mc_items = [
            ("Mean (평균)", mc.mean),
            ("Median (중위수)", mc.median),
            ("Std (표준편차)", mc.std),
            ("5th Percentile", mc.p5),
            ("25th Percentile", mc.p25),
            ("75th Percentile", mc.p75),
            ("95th Percentile", mc.p95),
            ("Min", mc.min_val),
            ("Max", mc.max_val),
        ]
        ...
        if mc.histogram_bins:
            r += 1
            write_cell(ws, r, 1, "주당가치 분포 (히스토그램)", font=SECTION_FONT)
            r += 1
    ...
    if sc_header_row and ctx.sc_codes:
        ...
        cats1 = Reference(
            ws,
            min_col=1,
            min_row=sc_header_row + 1,
            max_row=sc_header_row + len(ctx.sc_codes),
        )
        vals1 = Reference(
            ws,
            min_col=2,
            min_row=sc_header_row,
            max_row=sc_header_row + len(ctx.sc_codes),
        )
    ...
    if (
        ev_data_start is not None
        and ev_data_end is not None
        and ev_data_end >= ev_data_start
    ):
        chart2 = BarChart()
        chart2.type = "col"
        chart2.style = 10
...
def _write_football_field(ws, r: int, ctx: Ctx):
    ...
    write_cell(ws, r, 1, "Football Field — 밸류에이션 범위", font=SECTION_FONT)
    r += 1
    FF_COL_START = 8
    ...
    cats3 = Reference(
        ws, min_col=FF_COL_LABEL, min_row=ff_header_row + 1, max_row=ff_data_end
    )
    vals_lo = Reference(
        ws, min_col=FF_COL_LO, min_row=ff_header_row + 1, max_row=ff_data_end
    )
    vals_range = Reference(
        ws, min_col=FF_COL_RANGE, min_row=ff_header_row + 1, max_row=ff_data_end
    )
