...
- **NEVER run `git checkout -- <file>`, `git show HEAD:<f> > <f>`, `git restore`, or `git reset --hard` on this repo without first confirming the target file is clean.** The working tree carries a large body of UNCOMMITTED work (main/HEAD is an old baseline; ~50 files show `M`, and some feature files like `engine/investability_gate.py` are untracked). Overwriting a dirty file with its HEAD blob permanently destroys uncommitted changes — there is no stash/reflog copy. If a file is corrupted, back it up (`cp <f> /tmp/bak`) and reconstruct; do NOT reach for HEAD.
