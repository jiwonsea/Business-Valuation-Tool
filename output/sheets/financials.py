...
def sheet_financials(ctx: Ctx):
    ...
    if ctx.method == "sotp" and ctx.result.da_allocations:
        ...
        fs_title = (
            "부문별 재무 — D&A 배분 (금융 부문 제외)"
            if _has_mixed_fs
            else "부문별 재무 — 유무형자산 비중 D&A 배분"
        )
        write_cell(ws, r, 1, fs_title, font=TITLE_FONT)
        r += 1
        ...
        for yr in reversed(years):
            if yr not in ctx.result.da_allocations:
                continue
            r += 1
            write_cell(ws, r, 1, f"── {yr}년 ──", font=SECTION_FONT)
            r += 1
            if _has_mixed_fs:
                seg_headers = [
                    "부문",
            ...
            if _has_mixed_fs:
                write_cell(
                    ...
                    r,
                    3,
                    sum(ctx.vi.segment_data[yr][c]["revenue"] for c in ctx.seg_codes),
                    fmt=NUM_FMT,
                    bold=True,
                )
                write_cell(
                    ...
                    r,
                    4,
                    sum(ctx.vi.segment_data[yr][c]["op"] for c in ctx.seg_codes),
                    fmt=NUM_FMT,
                    bold=True,
                )
                write_cell(
                    ...
                    r,
                    5,
                    sum(ctx.vi.segment_data[yr][c]["assets"] for c in ctx.seg_codes),
                    fmt=NUM_FMT,
                    bold=True,
                )
                ...
                write_cell(
                    ...
                    r,
                    7,
                    sum(alloc[c].da_allocated for c in ctx.seg_codes),
                    fmt=NUM_FMT,
                    bold=True,
                )
                write_cell(
                    ...
                    r,
                    8,
                    sum(alloc[c].ebitda for c in ctx.seg_codes),
                    fmt=NUM_FMT,
                    bold=True,
                )
            else:
