...
p_no_override = DCFParams(
    ebitda_growth_rates=[0.08] * 5, tax_rate=22.0,
    capex_to_da=1.5, terminal_growth=2.5
)
...
r_no  = calc_dcf(ebitda_base=1000, da_base=500, revenue_base=8000, wacc_pct=10.0, params=p_no_override)
# With override: ratio=0.22 regardless of da_base passed in
r_yes = calc_dcf(ebitda_base=1000, da_base=500, revenue_base=8000, wacc_pct=10.0, params=p_with_override)

check("Override produces different EV than single-year (fix active)",
      r_yes.ev_dcf != r_no.ev_dcf,
      f"EV(single-yr D&A=50%)={r_no.ev_dcf:,}  EV(3yr avg D&A=22%)={r_yes.ev_dcf:,}")
check("3yr avg D&A/EBITDA=22% used in projection (lower D&A -> lower CapEx -> higher FCFF)",
      r_yes.ev_dcf > r_no.ev_dcf,
      f"FCFF is higher when CapEx = 1.5x * lower DA")
# Verify the override actually flowed through
proj_da_ratio = r_yes.projections[0].da / r_yes.projections[0].ebitda
check(f"Projection year 1 D&A/EBITDA ratio ~0.22 (override applied)",
...
check("Old D/E (total liab) was inflated above 300%",
      old_de > 300,
      f"old D/E={old_de}%  (includes trade payables, accruals, etc.)")
check("New D/E (gross_borr) is realistic for typical Korean mid-cap",
      50 < new_de < 200,
      f"new D/E={new_de}%")

old_eq_w = round(100 / (1 + old_de / 100), 1)
new_eq_w = round(100 / (1 + new_de / 100), 1)
p_old_de = WACCParams(rf=3.5, erp=7.0, bu=0.75, de=old_de, tax=25.0, kd_pre=5.5, eq_w=old_eq_w)
p_new_de = WACCParams(rf=3.5, erp=7.0, bu=0.75, de=new_de, tax=25.0, kd_pre=5.5, eq_w=new_eq_w)
r_old_de = calc_wacc(p_old_de)
r_new_de = calc_wacc(p_new_de)

check("WACC is materially different between old and new D/E",
...
check("Old inflated D/E was causing suppressed WACC (debt-weight dominance)",
...
ebitda = 12_000; da = 4_000; rev = 97_700; net_debt = 1_000; shares = 3_200

p_default = DCFParams(
    ebitda_growth_rates=[0.15, 0.12, 0.09, 0.06, 0.04],
    tax_rate=15.0, capex_to_da=1.1, terminal_growth=2.5
)
p_calibrated = DCFParams(
    ebitda_growth_rates=[0.15, 0.12, 0.09, 0.06, 0.04],
    tax_rate=15.0, capex_to_da=2.1, terminal_growth=2.5
)
r_def = calc_dcf(ebitda_base=ebitda, da_base=da, revenue_base=rev, wacc_pct=9.5, params=p_default)
r_cal = calc_dcf(ebitda_base=ebitda, da_base=da, revenue_base=rev, wacc_pct=9.5, params=p_calibrated)

ps_def = (r_def.ev_dcf - net_debt) / shares
ps_cal = (r_cal.ev_dcf - net_debt) / shares

check("Default 1.1x capex ratio significantly overestimates EV vs calibrated 2.1x",
      r_def.ev_dcf > r_cal.ev_dcf * 1.5,
      f"EV(1.1x)=${r_def.ev_dcf:,.0f}M  EV(2.1x)=${r_cal.ev_dcf:,.0f}M")
check("Per-share gap is material (>$10 per share)",
      (ps_def - ps_cal) > 10,
      f"per share: 1.1x=${ps_def:.0f}  2.1x=${ps_cal:.0f}  gap=${ps_def-ps_cal:.0f}")
check("Both still far below market $361 (optionality premium not in EBITDA DCF)",
      ps_cal < 361 * 0.5,
...
base = DCFParams(
    ebitda_growth_rates=[0.05] * 5, tax_rate=22.0,
    capex_to_da=1.5, terminal_growth=2.5,
    da_to_ebitda_override=0.28
)
check("da_to_ebitda_override field exists on DCFParams",
...
sc = ScenarioParams(
    code="Bull", name="Bull Case", prob=25, ipo="N/A",
    shares=1_000_000, growth_adj_pct=20, terminal_growth_adj=0.3
)
sc_params = _make_scenario_dcf_params(base, sc, wacc=9.5)
check("Scenario DCFParams carries da_to_ebitda_override from base",
