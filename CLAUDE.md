...
pytest tests/                                             # 테스트
pip install -e ".[dev,pipeline,ai,ui]"                    # 의존성 설치
```

## Workflow Rules

1. **생각과 실행 분리**: 비자명한 작업은 구조/방식을 먼저 제안하고, 확인 후 실행.
2. **방법론 자동 선택**: `engine/method_selector.py`가 부문 수, 업종에 따라 SOTP/DCF/DDM 분기. 수동 지정(`valuation_method`) 우선.
...
- 환경변수: `ANTHROPIC_API_KEY`, `DART_API_KEY`, `NAVER_CLIENT_ID`, `NAVER_CLIENT_SECRET`
