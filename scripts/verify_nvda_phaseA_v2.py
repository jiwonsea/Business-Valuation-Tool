...
def main():
    ...
    chk(
        "beta status = consumed_blume",
        bp.get("status") == "consumed_blume",
        str(bp.get("status")),
    )
    ...
    chk(
        "Blume math: 0.67*2.040+0.33 → unlever = bu",
        abs(bu_calc - w["bu"]) < 0.003,
    ...
    for f in (
        ...
        chk(
            "provenance 필드 '%s' 값 기록" % f,
            f in bp,
            "누락 (감사 시 캐시 재계산 필요)",
        )
    chk(
        "source_hash 존재", bool(bp.get("source_hash")), str(bp.get("source_hash"))[:16]
    )
    ...
    if cache:
        ...
        chk(
            "캐시 observation_count == 프로필",
            n_stock == bp.get("observation_count")
            or n_stock + 1 == bp.get("observation_count"),
    ...
    chk(
        "CV convergence = 0/25",
        getattr(q, "cv_convergence", None) == 0,
        str(getattr(q, "cv_convergence", None)),
    )
    chk(
        "품질 total ≈ 45 이하 (gate 전) 또는 0 (gate 후)",
        getattr(q, "total", 99) <= 45,
        "total=%s grade=%s" % (getattr(q, "total", "?"), getattr(q, "grade", "?")),
    )
    ...
    chk(
        "넥써쓰 AR 5503/12440/15564",
        ar == {"Bear": 5503, "Base": 12440, "Bull": 15564},
        str(ar),
    )
    chk("넥써쓰 확률가중 362", wsum == 362, str(wsum))
    chk(
        "넥써쓰 Base equity 36,211",
        xr.scenarios["Base"].equity_value == 36211,
        str(xr.scenarios["Base"].equity_value),
    )
    chk(
        "넥써쓰 net_debt 51,687",
        xr.scenarios["Base"].net_debt == 51687,
