...
CHANGED = [
    "schemas/models.py", "engine/scenario.py", "engine/monte_carlo.py",
    "valuation_runner.py", "output/sheets/scenarios.py",
    "output/sheets/dashboard.py", "output/console_report.py",
]
...
def p1_ast():
    ...
    nul = []
    for p in ROOT.rglob("*"):
        if (p.is_file() and p.suffix in {".py", ".yaml", ".md", ".sql"}
...
def r_nexus():
    ...
    res = run_valuation(apply_net_debt_gate(
        load_profile(str(ROOT / "profiles" / "nexus.yaml"))))
    ...
    for code, seg in (res.sotp or {}).items():
        print("    %-14s ev=%-9s multiple=%-8s method=%s"
    ...
    for code, s in res.scenarios.items():
        print("    [%s] total_ev=%-9s equity=%-9s ps=%-6s weighted=%s"
    ...
    for code, s in res.scenarios.items():
        if code not in EXPECT_PS:
            continue
        chk("R-2 %s %d원" % (code, EXPECT_PS[code]),
            abs(s.post_dlom - EXPECT_PS[code]) <= 1, "%d원" % s.post_dlom)
        chk("R-7 %s AR=%d" % (code, EXPECT_AR[code]),
            s.receivable_recovery_value == EXPECT_AR[code],
            str(s.receivable_recovery_value))
        if code == "Base":
            dbl = s.equity_value > 40000
            chk("R-7 Base equity ~36,211 (이중가산 검출)",
                abs(s.equity_value - EXPECT_BASE_EQUITY) <= 2,
                "%d%s" % (s.equity_value, "  <<< 이중가산!" if dbl else ""))

    w = sum(s.weighted for s in res.scenarios.values())
    chk("R-3 확률가중 %d원" % EXPECT_WEIGHTED, abs(w - EXPECT_WEIGHTED) <= 1, "%d원" % w)

    if res.monte_carlo:
        mc = res.monte_carlo
        chk("R-11 MC 실행", True, "mean=%s pct_negative=%s"
            % (mc.mean, getattr(mc, "pct_negative", "N/A")))
    else:
    ...
    for name in ("nvda", "tsla"):
        ...
        try:
            ...
            if gd is None:
                ...
                print("    [진단] market_comparison=%s" % ("None" if mc is None else
                      "price=%s gap=%s" % (mc.market_price, mc.gap_ratio)))
                ...
                if mc is None:
                    print("  [ENV SKIP] R-8 %s: live market comparison unavailable" % name)
                    continue
            chk("R-8 %s gap_diagnostic 생존" % name, gd is not None,
...
def r9_backcompat():
    ...
    for p in others:
        try:
            ...
            bad += 1
    chk("R-9 하위호환 (%d개)" % len(others), bad == 0,
...
def r10_pytest():
    ...
    r = subprocess.run(
        [sys.executable, "-m", "pytest", "tests/", "-q",
         "--deselect", "tests/test_engine.py::TestScenarioDriverRoundTrip"],
