...
- **Cowork/Linux-VM bridge Git reads must use `git --no-optional-locks ...`** to prevent a stale `index.lock`; this applies to read-only bridge commands, not local Git writes.
