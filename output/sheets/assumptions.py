...
def sheet_assumptions(ctx: Ctx):
    ...
    write_cell(
        ws, 1, 1, f"{ctx.vi.company.name} 기업가치평가 — 핵심 가정값", font=TITLE_FONT
    )
    ...
    if ctx.method in ("ddm", "rim"):
        ...
        write_cell(ws, r, 1, "자기자본비용 (Ke)", font=SECTION_FONT)
        r += 1
        ke_params = [
            ...
            (
                "Beta (βL)",
                f"{w.bl:.3f}",
                "Equity Beta" if wp.is_financial else "Hamada 조정",
            ),
        ...
        write_cell(ws, r, 1, "WACC 구성요소", font=SECTION_FONT)
        r += 1
        ...
        if wp.size_premium > 0:
            ke_idx = next(
                i for i, p in enumerate(wacc_params) if p[0].startswith("자기자본비용")
            )
            wacc_params.insert(
                ke_idx + 1,
                (
                    "  비상장/규모 프리미엄 (SP)",
                    f"{wp.size_premium:.2f}%",
                    "비상장 할인 반영",
                ),
            )
    ...
    if ctx.method == "sotp":
        ...
        write_cell(ws, r, 1, "부문별 적용 멀티플", font=SECTION_FONT)
        r += 1
        ...
        write_cell(ws, r, 1, "DCF 핵심 가정", font=SECTION_FONT)
        r += 1
        dcf_items = [
            (
                "영구성장률 (Terminal Growth)",
                f"{dcf_p.terminal_growth:.1f}%",
                "Gordon Growth",
            ),
        ...
        if ddm_p:
            write_cell(ws, r, 1, "DDM 파라미터", font=SECTION_FONT)
            r += 1
            ddm_items = [
                (
                    "주당 배당금 (DPS)",
                    f"{ddm_p.dps:,.0f}{ctx.currency_sym}",
                    "최근 실적",
                ),
            ...
            if ddm_p.buyback_per_share > 0:
                ddm_items.append(
                    (
                        "주당 자사주매입",
                        f"{ddm_p.buyback_per_share:,.0f}{ctx.currency_sym}",
                        "Total Payout",
                    )
                )
        ...
        if rim_p:
            write_cell(ws, r, 1, "RIM 파라미터", font=SECTION_FONT)
            r += 1
            rim_items = [
                (
                    "영구성장률 (Terminal)",
                    f"{rim_p.terminal_growth:.2f}%",
                    "RI 영구성장률",
                ),
        ...
        if nav_p:
            write_cell(ws, r, 1, "NAV 가정", font=SECTION_FONT)
            r += 1
        ...
        write_cell(ws, r, 1, "적용 멀티플", font=SECTION_FONT)
        r += 1
    ...
    if ctx.method == "sotp" and ctx.result.dcf is not None:
        dcf_p = ctx.vi.dcf_params
        write_cell(ws, r, 1, "DCF 교차검증 가정", font=SECTION_FONT)
        r += 1
        dcf_items = [
        ...
        if dcf_p.terminal_ev_ebitda is not None:
            dcf_items.append(
                (
                    ...
                    f"{dcf_p.terminal_ev_ebitda:.1f}x",
                    "터미널밸류 교차검증",
                )
            )
    ...
    has_dlom = (
        any(ctx.vi.scenarios[c].dlom > 0 for c in ctx.sc_codes)
        if ctx.sc_codes
        else False
    )
    ...
    if ctx.sc_codes:
        r += 1
        section_title = "시나리오 확률" + (
            " / DLOM" if not is_listed and has_dlom else ""
        )
        ...
        style_header_row(ws, r, 1 + len(ctx.sc_codes))
        r += 1
        ...
        if not is_listed:
            ...
            for i, sc_code in enumerate(ctx.sc_codes, 2):
                write_cell(
                    ws, r, i, f"{ctx.vi.scenarios[sc_code].dlom}%", fill=BLUE_FILL
                )
            r += 1
    ...
    if has_cps or has_rcps:
        ...
        if has_cps:
            ...
            write_cell(
                ...
                r,
                2,
                f"{rate:.1f}%" if rate > 0 else "무배당 (zero-coupon)",
                fill=BLUE_FILL,
            )
            r += 1
        if has_rcps:
            ...
            write_cell(ws, r, 3, "상환전환우선주")
            r += 1
            write_cell(ws, r, 1, "RCPS 만기 (년)")
            write_cell(ws, r, 2, f"{ctx.vi.rcps_years}년", fill=YELLOW_FILL)
            r += 1
            write_cell(ws, r, 1, "RCPS 배당률")
            write_cell(ws, r, 2, f"{ctx.vi.rcps_dividend_rate:.1f}%", fill=YELLOW_FILL)
            write_cell(ws, r, 3, "배당 지급 중 → 상환액 ≠ 단순 복리")
            r += 1

    # ── Other parameters ──
    r += 1
    write_cell(ws, r, 1, "기타 파라미터", font=SECTION_FONT)
    r += 1
    ...
    if _is_mixed:
        ...
        write_cell(ws, r, 1, "(-) 금융부문 부채 (PBV 내재)")
        write_cell(ws, r, 2, fin_debt, fmt=NUM_FMT, fill=BLUE_FILL)
        r += 1
        write_cell(ws, r, 1, "유효 순차입금")
    ...
    if ctx.vi.company.shares_preferred > 0:
        ...
        write_cell(
            ws, r, 2, ctx.vi.company.shares_preferred, fmt=NUM_FMT, fill=BLUE_FILL
        )
        r += 1
    ...
    if ctx.vi.company.treasury_shares > 0:
        ...
        write_cell(
            ...
            r,
            2,
            ctx.vi.company.shares_outstanding,
            fmt=NUM_FMT,
            fill=BLUE_FILL,
            bold=True,
        )
