...
- Cross-validation DCF calls in non-DCF methods (DDM/RIM/NAV) are unguarded — `calc_dcf()` raises `ValueError` on `ebitda<=0` or `WACC<=TG`. Wrap in try/except when adding new cross-validation paths.
