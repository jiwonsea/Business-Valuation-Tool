...
## 탐지 결과 / Detection Results

ECOS 실데이터 기준 (2022-01-03 ~ 2025-12-31, 981 영업일):

| 탐지 유형 | 건수 | 비율 |
|-----------|------|------|
| IQR 이상치 (롤링 60일) | 350건 | 35.7% |
| 전일 대비 변동폭 초과 (만기별 차등) | 555건 | 56.6% |
| 양방향 동시 플래그 (IQR + bp) | 82건 | 8.4% (최고 신뢰도) |
| **이상치 이벤트 총계** | **823건** | |

양방향 동시 플래그(BOTH) 82건은 2022년 한은 급격한 기준금리 인상 사이클 구간에 집중됨 — 실제 시장 충격 구간과의 높은 일치를 확인.
